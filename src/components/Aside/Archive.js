...
import createDatesDisplay from "../Header/formatDates";
...
export default function Archive() {
  ...
  const { data, loading } = useFetch({
    api: "https://fs-blog-backend.fly.dev/blog",
    method: "GET",
  });
      ...
      {dates &&
        Object.keys(dates).map((year) => (
          <div className="archive" key={year}>
            <p
              className="clickable archive-button"
              onClick={
                expandArchive
                  ? () => setExpandArchive(null)
                  : () => setExpandArchive(true)
              }
            >
              Archive
              {expandArchive === true ? " ▲" : " ▼"}
